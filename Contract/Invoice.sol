...
contract Escrow {
    ...
    address public shipingCompanyAddress;

    constructor(address _shippingCompanyAddress) {
      shippingCompanyAddress = _shippingCompanyAddress;
    }

    function sendAmount(uint amount) public payable {
        payable(shippingCompanyAddress).transfer(amount);
    }
}
...
contract Invoice is ERC721URIStorage {
    ...
    function createInvoiceToken(
        ...
        address _shippingCompanyAddress
    ) public payable onlyOwner returns(uint256){

        // creat an escrow account for this nft invoice
        Escrow  newContract = new Escrow(_shippingCompanyAddress);
